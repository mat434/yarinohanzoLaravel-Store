...
<div class="card h-100 custom-product-card border-0 shadow-sm d-flex flex-column justify-content-between"
    style="width: 18rem;">
    ...
    <div class="position-relative overflow-hidden custom-card-img-container">
        ...
        @if(isset($discount))
            <span
                class="position-absolute top-0 start-0 bg-danger text-white px-3 py-1 text-uppercase fw-bold custom-discount-badge">
                Offerta
            </span>
        @endif
    </div>
        ...
        <div>
            <div class="mb-3 d-flex align-items-baseline gap-2">
                @if (isset($discount))
                    ...
                    <span
                        class="text-muted text-decoration-line-through small">{{ number_format($katana['prezzo'], 2, ',', '.') }}€</span>
                @else
            ...
            <div class="d-flex gap-2">
                <a href="{{ $urlCorretto }}"
                    class="btn btn-dark w-100 text-uppercase fw-bold custom-btn-square py-2 letter-spacing-1">
                    Dettagli
                </a>
                <form action="{{ route('cart.add') }}" method="POST" class="m-0">
                    ...
                    <button type="submit" class="btn btn-outline-dark custom-btn-square py-2 px-3"
